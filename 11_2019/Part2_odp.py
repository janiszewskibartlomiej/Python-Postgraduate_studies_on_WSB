...
#---- load csv ----
metadata = pd.read_csv('SampleFiles\\Metadane1.csv')

#---- load txt ----
obs = pd.read_csv('SampleFiles\\Sample.txt', sep = "\t", header = None)
obs.columns = ['ID', 'ObsDate', 'Value', 'RelDate']

#--- load xls(x) ----
metadata1 = pd.read_excel('SampleFiles\\Metadane1.xlsx', sheet_name = 'Metadane1' )
...
engine = create_engine('sqlite:///SampleFiles//SampleDB.db')

# STEP 2: Declare what to retrive from db. SQL query
my_query = 'SELECT * FROM Obs'

# STEP 3a: Connect to db and retrive data (write own query)
obs_from_sql = pd.read_sql_query(my_query, engine)

# STEP 3b: Connect to db and retrive data (get whole table)
obs_from_sql1 = pd.read_sql_table('Obs', engine)
